<template>
    ...
    >
        <div ref="reference" :class="[prefixCls + '-rel']">
            <slot>
                <div class="trade-input-wrapper trade-input-type"
                ...
                >
                    ...
                    <i @click="handleIconClick"
                       class="icon iconfont trade-select-arrow"
                       :class="arrowTypeClasses"
                       slot="suffix"
                    />
                    <!-- :style="{ transform: `rotate(${(!visible && !(type && (type === 'time' || type === 'timerange'))) ? '0' : '180'}deg)` }" -->
                </div>
            </slot>
        </div>
...
const prefixCls = 'trade-date-picker';
const pickerPrefixCls = 'trade-picker';
...
const pulseElement = (el) => {
    const pulseClass = 'trade-date-picker-btn-pulse';
...
export default {
    ...
    data(){
        const isRange = this.type ? this.type.includes('range') : false;
    ...
    computed: {
        ...
        publicVModelValue(){
            if (this.multiple) {
                ...
                if (this.type && this.type.match(/^time/)) val = val.map(this.formatDate);
                return (isRange || this.multiple) ? val : val[0];
            }
        },
        publicStringValue(){
            const {formatDate, publicVModelValue, type} = this;
            if (type && type.match(/^time/)) return publicVModelValue;
        ...
        arrowTypeClasses () {
            ...
            if (this.type === 'time' || this.type === 'timerange') {
                ...
                // type = 'icon-arrow-down-bold';
                type = 'icon-Rectangle';
            }

            if (this.showClose) type = 'icon-icon_big_fail';
    ...
    methods: {
        onSelectionModeChange(type){
            if (type && type.match(/^date/)) type = 'date';
        ...
        navigateDatePanel(direction, shift){
            ...
            if (shift){
                ...
                const button = this.$refs.drop.$el.querySelector(`.trade-date-picker-${position}-btn-arrow${double}`);
        ...
        handleInputChange (event) {
            const isArrayValue = this.type ? this.type.includes('range') : this.multiple;
